		...
		<view class="product-window" :class="(attr.cartAttr === true ? 'on' : '') + ' ' + (iSbnt || destri?'join':'') + ' ' + (isPresell ?'presell-window':'product-window')">
			<view class="textpic acea-row row-between-wrapper">
				...
				<view class="text">
					...
					<view v-if="isPresell" class="money presell_price">
						<view>
							预售价
							{{unit}}<text class="num">{{ attr.productSelect.price }}</text>
							<text v-if="presell_type === 2">定金{{unit}}<text class="num">{{ attr.productSelect.down_price }}</text></text>
						</view>
			...
			<!-- <view class="cart acea-row">
				...
				<view class="carnum acea-row row-left">
					...
					<view class='item num'>
						<input type="number" v-model="attr.productSelect.cart_num" data-name="productSelect.cart_num" disabled @input="bindCode(attr.productSelect.cart_num)"></input>
					</view>
					<view v-if="iSplus" class="item plus" :class=" attr.productSelect.cart_num >= attr.productSelect.stock || destri ? 'on' : ''" @click="CartNumAdd">+</view>
					<view v-else class='item plus' :class='(attr.productSelect.cart_num >= attr.productSelect.quota_show) 
															|| (attr.productSelect.cart_num >= attr.productSelect.product_stock)? "on":""' @click='CartNumAdd'>+</view>
				</view>
			</view> -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			CartNumAdd: function() {
				if (!this.destri && this.attr.productSelect.cart_num < 3) {
