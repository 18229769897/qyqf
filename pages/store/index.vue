			...
			<view class="page-index skeleton" :class="{ bgf: navIndex > 0 }">
				<block>
					...
					<view v-for="(item, index) in styleConfig" :key="index">
						<block v-if="item.name != 'headerSerch' && item.name != 'tabNav' && navIndex == 0">
							<component
								:is="item.name"
								:index="index"
								:isFixed="isFixed"
					...
					<block v-for="(item, index) in styleConfig" :key="index" >
						<block v-if="navIndex == 0">
							...
							<bargain v-if="item.name == 'bargain'" :merId="mer_id" :dataConfig="item" @changeBarg="changeBarg"></bargain>
						...
						<!-- <customMenu v-if="item.name == 'customMenu'" :merId="mer_id" :dataConfig="item" class="tabNav" :tabTitle="navTop"
						@bindHeight="bindHeighta" :isFixed="isFixed"></customMenu> -->
					</block>
					<!-- #endif -->
					<block v-if="navIndex == 0">
						<view class="main">
...
import customMenu from '../index/component/customMenu';
...
export default {
	computed: configMap({hide_mer_status:0,site_logo:'',community_status:0,share_title:''},
	...
	methods: {
		...
		diyData() {
			let that = this;
			getDiy({mer_id: this.mer_id}).then(res => {
				setTimeout(() => {
					that.isNodes++;
				}, 0);
		...
		get_host_product: function() {
			...
			that.hotTitle = '';
			getMerProductHot(that.mer_id,{page:that.hotPage, limit:that.hotLimit}).then(res => {
				let list = res.data.list;
