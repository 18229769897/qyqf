						...
						<view class='item' v-for="(item,index) in orderList" :key="index">
							<view @click='goOrderDetails(item.group_order_id)'>
										...
										<block v-for="(goods,g) in order.orderProduct">
											<view class='item-info acea-row row-between row-top'>
												...
												<view class='text acea-row row-between'>
													...
													<view class='money'>
														<view>积分{{goods.cart_info.productPresellAttr.presell_price}}</view>
														<view>x{{goods.product_num}}</view>
													</view>
												</view>
												<view v-if="goods.cart_info.productPresell.presell_type === 2" class="event_price">
													定金待支付 <text class="t-color">积分{{ order.pay_price }}</text>
													尾款待支付 <text class="t-color">积分{{ order.presellOrder.pay_price }}</text>
												</view>
											</view>
										</block>
									</view>
											...
											<view class='item-info acea-row row-between row-top'>
												...
												<view class='text acea-row row-between'>
													...
													<view class='money' v-if="item.orderList[0].activity_type == 4">
														<view v-if="goods.cart_info.activeSku">积分{{goods.cart_info.activeSku.active_price}}
														</view>
														<view>x{{goods.product_num}}</view>
													</view>
													<view v-else class='money'>
														<view>积分{{goods.cart_info.productAttr.price}}</view>
														<view>x{{goods.product_num}}</view>
													</view>
												</view>
											</view>
								...
								<view class='totalPrice' v-if="item.orderList[0].activity_type !== 2">共{{item.total_num || 0}}件商品，总金额
									<text class='money p-color'>积分{{item.pay_price}}</text>
								</view>
						...
						<view class='item' v-for="(item,index) in orderList" :key="index">
							<view @click='goOrderDetails(item.order_id)'>
										...
										<view class='text acea-row row-between'>
											...
											<view class='money' v-if="item.activity_type == 3">
												<view v-if="goods.cart_info.productAssistAttr">
													积分{{goods.cart_info.productAssistAttr.assist_price}}</view>
												<view>x{{goods.product_num}}</view>
											</view>
											<view class='money' v-else-if="item.activity_type == 4">
												<view v-if="goods.cart_info.activeSku">积分{{goods.cart_info.activeSku.active_price}}</view>
												<view>x{{goods.product_num}}</view>
											</view>
								...
								<view class='totalPrice' v-if="item.activity_type == 2">共{{item.orderNum || 0}}件商品，总金额
									<text class='money p-color'>积分{{item.presell_price}}</text>
								</view>
