<template>
	<view :style="viewColor">
		<view class='my-order'>
			...
			<view class='nav nav2 acea-row row-around'>
				<view class='item' v-for="item in tabList" :class="{'on':group_id==item.group_id}" @click="onChange(item)">
					<view>{{item.text}}</view>
				</view>
			</view>
				...
				<view v-if="orderList.length > 0">
									...
									<view v-if="order.activity_type === 2">
										<block v-for="(goods,g) in order.orderProduct">
											<view class='item-info acea-row row-between row-top'>
												<view class='pictrue'>
													<image :src='(goods.cart_info.productAttr && goods.cart_info.productAttr.image) || goods.cart_info.product.image'>
													</image>
												</view>
											...
											<view class='item-info acea-row row-between row-top'>
												...
												<view class='text acea-row row-between'>
													...
													<view class='money' v-if="item.orderList[0].activity_type == 4">
														<view v-if="goods.cart_info.activeSku">￥{{goods.cart_info.activeSku.active_price}}
														</view>
														<view>x{{goods.product_num}}</view>
													</view>
					...
					<block v-else>
						<view class='item' v-for="(item,index) in orderList" :key="index">
							<view @click='goOrderDetails(item.order_id)'>
								<view class='title acea-row row-between-wrapper'>
									<view v-if="item.status == 0" class='t-color'>
										<text v-if="item.stage_status==0 && item.user_check==0">等待中</text>
										<text v-else>待发货</text>
									</view>
									<view class="ml-auto" v-if="item.range_status==0&&item.pay_type==0">轮次:{{item.round}}</view>
									<view v-if="item.status == 1" class='t-color'>待收货</view>
									<view v-if="item.status == 2" class='t-color'>待评价</view>
									<view v-if="item.status == 3" class='t-color'>
										<text v-if="item.is_send_haiwen == 1">，已给票</text>
							...
							<view class='bottom acea-row row-right row-middle'>
								<view class='bnt b-color' v-if="(item.status==0&&item.user_check==0&&item.stage_stauts==10&&item.paid==1)" @click='shipments(item.order_id)'>选择发货
								</view>
								<view class='bnt b-color' v-if="(item.status==0&&item.user_check==0&&item.stage_stauts==10&&item.paid==1)" @click='exchangeHaiQuan(item.order_id)'>
									兑换嗨券</view>
								...
								<block v-if="item.status == 2">
									<navigator v-if="community_status == 1 && !item.community_id" :url="'/pages/plantGrass/plant_release/index?order_id='+item.order_id"
		...
		<payment :payMode='payMode' :pay_close="pay_close" @onChangeFun='onChangeFun' :order_id="pay_order_id" :totalPrice='totalPrice'>
		</payment>
	</view>
</template>
	...
	export default {
		...
		data() {
			return {
				...
				tabList: [
					{ group_id: "", text: "全部" },
					{ group_id: 1, text: "新手专区" },
					{ group_id: 2, text: "体验专区" },
					{ group_id: 3, text: "恒大专区" },
				]
			};
		},
		onReady() {
		...
		methods: {
			...
			// 选择发货
			async shipments(order_id) {
				if (!(await this.$msg.alert("是否要选择发货吗"))) return;
				try {
					let { message } = await sendGoods(order_id);
			...
			// 兑换嗨券
			async exchangeHaiQuan(order_id) {
				if (!(await this.$msg.alert("是否要将该订单兑换为嗨券"))) return;
				try {
					let { message } = await exchangeHaiQuanApi(order_id);
					this.$msg.succ(message);
					this.getOrderData();
					this.getOrderList();
					this.getUserInfo();
			...
			async getOrderList(options = {}) {
				...
				if (this.orderStatus == 0) {
					...
					try {
						let { data: { list } } = await groupOrderList(params, option);
					...
					try {
						let { data: { list } } = await getOrderList(params, option);
						this.setList("orderList", list, option.first);
						this.getProductCount();
					} catch (e) {
		...
		onReachBottom: function() {
			this.getOrderList({ first: false, loading: false });
		}
	}
</script>
