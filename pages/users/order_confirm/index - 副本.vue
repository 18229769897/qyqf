<template>
	<view :style="viewColor">
					...
					<view v-for="(item,index) in order_extend">
						...
						<view v-if="item.key == 'image'" class='item'>
							...
							<view class="upload">
								...
								<view class='pictrue acea-row row-center-wrapper row-column' @click='uploadpic(item)'
									v-if="pics.length < 5">
		...
		<block v-if="coupon.status">
			<couponListWindow :coupon='coupon' :couponTitle="plantCoupon ? '平台优惠券' : '优惠券'" @ChangCouponsClone="ChangCouponsClone"
				@getCoupon="getCoupon" :openType='openType' @ChangCoupons="ChangCoupons" :coupon_amount='coupon_amount'
		...
		<block v-if="isShowBox">
			<checkDelivery :deliveryName="deliveryName" :isShowBox="isShowBox" :activeObj="activeObj" @close="boxClose"
				@confirmBtn="getData">
			</checkDelivery>
		</block>
		<!--优惠明细弹窗-->
		...
		methods: {
			...
			getConfirm: function(address_id) {
				...
				getOrderConfirm({
					...
					this.product_type = res.data.order[0].list[0].product_type;
					// 默认选中
					...
					that.$set(that.coupon, "coupon", that.plantCoupon ? res.data.platformCoupon : res.data.order[that.couponIndex]
						.coupon);
					that.$set(that, "coupon_number", res.data.order[that.couponIndex].order.useCouponIds
						.length + that.subCoupon['0'].length);
					that.$set(that, "coupon_amount", parseFloat(res.data.order[that.couponIndex].order.coupon_price) + parseFloat(
					...
					that.$set(that, 'order_extend', (that.order_extend && that.order_extend.length > 0) ? that.order_extend : res
						.data.order_extend);
