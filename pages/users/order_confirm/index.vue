<template>
	<view :style="viewColor">
		<view class='order-submission'>
			<view v-if="order_model==0" class="allAddress" @click="onAddress">
				...
				<view class="store-item" v-for="(item,index) in cartInfo" :key="index">
						...
						<view v-if="goods.product_type == 2">
							<view class="product-item">
								...
								<view class="content event_content">
									<view class="name line1"><text class="event_name event_bg">预售</text>{{goods.productPresell.store_name}}
									</view>
									<view class="label" style="width: 70%;">{{goods.productAttr.sku}}</view>
									<view class="price">
										{{unit}}{{goods.productPresellAttr.presell_price}}
							...
							<view v-if="goods.productPresell.presell_type === 2" class="event_payTime">
								<view class="event_progress">
									<view class="progress_step">
										...
										<text class="price color_red">{{unit}}{{ (goods.productPresellAttr.down_price * goods.cart_num).toFixed(2) }}</text>
									</view>
									<view class="progress_step">
										<text class="name">尾款</text>
										<text class="price">{{unit}}{{ (goods.productPresellAttr.final_price * goods.cart_num).toFixed(2) }}</text>
									</view>
					...
					<view class="wrapper">
						<view class="boxs">
							<view class='item acea-row row-between-wrapper'>
								...
								<view class='discount' @tap="openShowBox(item,index)">
									{{item.order.isTake==0 ? deliveryName :'到店自提'}}
									<text v-if="item.delivery_way.length == 2" class='iconfont icon-jiantou'></text>
								</view>
							</view>
							<view class="store-address" v-if="product_type==3">
						...
						<view class='item acea-row row-between-wrapper' v-if='shippingType==0 && item.isTake == 0 && order_model == 0'>
							<view>快递费用 <text v-if="item.list[0].productPresell && item.list[0].productPresell.presell_type == 2">(尾款阶段）</text>
							</view>
							<view class='discount' v-if='item.order.postage_price > 0'>
								+{{unit}}{{item.order.postage_price}}
							</view>
						...
						<!-- <view class='item acea-row row-between-wrapper' v-if="!seckillId && order_type != 3 && order_type != 4 && item.order.enabledCoupon">
							...
							<block v-if="item.coupon.length>0">
								<view class='discount' @tap='couponTap(item,index)'>
									<text v-if="item.order.coupon_price>0">优惠{{unit}}{{item.order.coupon_price}}</text>
						...
						<view class='item acea-row row-between-wrapper' v-if="block_id>1">
							<view>使用红包积分</view>
							<template>
								<view v-if="userInfo.wait_released" class="flex justify-end">
									<view>
										<u-checkbox-group>
											<u-checkbox name="1" active-color="red" shape="circle" v-model="isUseRedpacket"></u-checkbox>
										</u-checkbox-group>
									</view>
								</view>
								<view v-else class='discount'>无可用红包积分</view>
							</template>
						</view>
						...
						<view class='item acea-row row-between-wrapper' v-if="textareaStatus">
							...
							<input v-if="coupon.status===false" placeholder-class='placeholder' @input='bindHideKeyboard' value="" name="mark" placeholder='选填备注信息'
								v-model="msgObj[item.mer_id]"></input>
						</view>
						<view class="total">
							共{{item.order.total_num}}件 小计
							<view class="price" v-if="item.isTake == 0"><text>{{unit}}</text>{{ item.order.pay_price }}
							</view>
							<view class="price" v-if="item.isTake == 1"><text>{{unit}}</text>{{ item.order.org_price}}
							</view>
						</view>
					</view>
				</view>
			...
			<!--虚拟商品-->
			<view class="wrapper virtual_form">
				<form report-submit='true'>
					<view v-for="(item,index) in order_extend">
						...
						<view v-if="item.key == 'image'" class='item'>
							...
							<view class="upload">
								<view class='pictrue' v-for="(itemn,indexn) in item.value" :key="indexn" :data-index="indexn" @click="getPhotoClickIdx">
									...
									<text class="iconfont icon-guanbi4" @click.stop="DelPic(indexn)"></text>
								</view>
			...
			<view class='wrapper'>
				<view class='item'>
					<view>支付方式</view>
					<template v-if="product_type==0">
						<payType2 :payType.sync="payType" :list="[`balance`]" />
					</template>
					<template v-if="product_type==3">
						<payType2 :payType.sync="payType" :list="[`integral`]" />
					</template>

					<template v-if="product_type==1">
						<payType2 :payType.sync="payType" :list="[`balance`,`buy_points`,`buy_points_and_balance`]" />
						<view v-if="payType=='buy_points_and_balance'" class="text-theme py-1">
							{{buy_points_rate}}商品积分=￥1
						</view>
					</template>

					<template v-if="product_type==4">
						<payType2 :payType.sync="payType" :list="[`balance`,`bean`,`balance_haiwen`]" />
					</template>

					<template v-if="product_type==10">
						<payType2 :payType.sync="payType" :list="[`hi_coupon`]" />
					</template>

					<template v-if="product_type==11">
						<payType2 :payType.sync="payType" :list="[`bean`]" />
					</template>
				</view>
			</view>
			<view class='moneyList'>
				...
				<!-- <view class='item acea-row row-between-wrapper'
					...
					<block v-if="platformCoupon.length > 0">
						<view class='discount money' @tap='couponTap2(platformCoupon,0)'>
							<text v-if="total_platform_coupon_price>0">优惠{{unit}}{{total_platform_coupon_price}}</text>
							<text v-else>暂未选择优惠券</text>
							<text class='iconfont icon-jiantou'></text>
						</view>
					</block>
				...
				<view class='item acea-row row-between-wrapper' v-if="priceGroup.storePostage > 0">
					<view>运费：</view>
					<view class='money'>+{{unit}}{{priceGroup.storePostage}}</view>
				</view>
			</view>
			...
			<view class='footer acea-row row-between-wrapper'>
				<view class="footer_count">
					<view>
						合计:
						<text class='pColor'>{{unit}}{{totalPrice || 0}}</text>
					</view>
				...
				<view class='settlement' :class='couponData.status != "noAddress" ? "" : "disabled"' style='z-index:100' @tap="SubOrder">
					{{couponData.status != "noAddress" ? '立即结算':'选择地址'}}
				</view>
			</view>
		</view>
		<block v-if="coupon.status">
			<couponListWindow :coupon='coupon' :couponTitle="plantCoupon ? '平台优惠券' : '优惠券'" @ChangCouponsClone="ChangCouponsClone" @getCoupon="getCoupon" :openType='openType'
				@ChangCoupons="ChangCoupons" :coupon_amount='coupon_amount' :coupon_number='coupon_number'>
			</couponListWindow>
		</block>
		<addressWindow ref="addressWindow" @changeTextareaStatus="changeTextareaStatus" :address='address' :pagesUrl="pagesUrl" @OnChangeAddress="OnChangeAddress"
		...
		<block v-if="isShowBox">
			<checkDelivery :deliveryName="deliveryName" :isShowBox="isShowBox" :activeObj="activeObj" @close="boxClose" @confirmBtn="getData">
			</checkDelivery>
		</block>
		<!--优惠明细弹窗-->
...
<script>
	...
	import { getStageDetailApi } from "@/api/store.js";
	export default {
		components: {
			couponListWindow,
			addressWindow,
		...
		onLoad: function(options) {
			...
			this.range_id = options.range_id;
		...
		methods: {
			...
			async getStageDetail() {
				try {
					let { data } = await getStageDetailApi({ stage_id: this.stage_id });
					this.stage = data;
				} catch (e) {
			...
			getConfirm: function(address_id) {
				...
				getOrderConfirm({
					...
					let orderInfo = res.data.order[0].list[0];
					this.product_type = orderInfo.product_type;
					this.buy_points_rate = orderInfo.product.buy_points_rate || buy_points_rate;
					switch (this.product_type) {
						case 0:
			...
			SubOrder: function(e) {
				...
				if (that.order_model == 1 && that.order_extend.length > 0) {
					for (var i = 0; i < that.order_extend.length; i++) {
						...
						if (el.require) {
							...
							} else if (el.key == 'email' && !/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/.test(el
									.value)) {
								return that.$util.Tips({
									title: '请输入正确的' + el.title
								});
							} else if (el.key == 'idCard' && !
				...
				if (this.block_id) {
					data.is_user_wait_released = this.isUseRedpacket ? (this.block_id > 1 ? 1 : 0) : 0;
				}
				...
				let price = parseFloat(that.totalPrice);
				let now_money = parseFloat(that.userInfo.now_money);
				let buy_points = parseFloat(that.userInfo.buy_points);
				let buy_points_rate = Number(this.buy_points_rate);
				if (!this.block_id || this.block_id == 1) {
					if (data.pay_type == 'balance' && now_money < price) {
						return this.$msg.fail("余额不足");
					}

					if (data.pay_type == 'buy_points' && buy_points / buy_points_rate < price) {
						return this.$msg.fail("商品积分不足");
					}

					if (data.pay_type == 'buy_points_and_balance' && ((buy_points / buy_points_rate) + now_money) < price) {
						return this.$msg.fail("余额不足");
					}

					if (data.pay_type == 'bean' && Number(this.userInfo.bean) < price) {
						return this.$msg.fail("嗨豆余额不足");
					}

					if (data.pay_type == 'hi_coupon' && Number(this.userInfo.hi_coupon) < price) {
						return this.$msg.fail("嗨券余额不足");
					}

					if (data.pay_type == 'balance_haiwen') {
						if (now_money < price * 0.4) {
							return this.$msg.fail("余额不足");
						}
						if (Number(this.userInfo.haiwen) < price * 0.6) {
							return this.$msg.fail("嗨票不足");
						}
					}
				} else {
					if (this.stage.new_logic == 1) {
						// 全部抢购区
						let max = Math.max(parseFloat(that.totalPrice) * 0.9, parseFloat(that.totalPrice) - parseFloat(
							that.userInfo.wait_released));
						if (data.pay_type == 'balance' && parseFloat(that.userInfo.now_money) < max) {
							return this.$msg.fail("余额不足");
						}
					} else if (this.block_id == 3 || this.block_id == 24) {
						// 鄂行天下
						let max = Math.max(parseFloat(that.totalPrice) * 0.902, parseFloat(that.totalPrice) - parseFloat(
							that.userInfo.wait_released) * 0.7);
						if (data.pay_type == 'balance' && parseFloat(that.userInfo.now_money) < max) {
							return this.$msg.fail("余额不足");
						}
					} else {
						// 其他抢购区
						let max = Math.max(parseFloat(that.totalPrice) * 0.79, parseFloat(that.totalPrice) - parseFloat(
							that.userInfo.wait_released) * 0.7);
						if (data.pay_type == 'balance' && parseFloat(that.userInfo.now_money) < max) {
							return this.$msg.fail("余额不足");
						}
				...
				if (data.pay_type != "weixin" && data.pay_type != "alipay") {
					this.show = true;
					this.$on("finish", (pwd) => {
						data.pay_pwd = pwd;
						that.payment(data);
