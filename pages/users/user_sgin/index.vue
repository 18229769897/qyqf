<template>
	<view :style="viewColor">
		<view class='sign'>
			...
			<view class='wrapper'>
				<scroll-view scroll-x :scroll-left="offsetLeft" @scroll="scroll">
					<view class='list scroll-box'>
						<view class='item scroll-item' v-for="(item,index) in signConfig" :key="index">
							<view>{{item.value.sign_day}}</view>
			...
			<view class='wrapper wrapper2'>
				...
				<view class="px-3 mt-2" style="line-height: 1.6;">
					<view>打卡说明：</view>
					<view>打卡签到福利仅对参与每日2场抢购用户发放</view>
					<view>打卡1日1元，累计叠加，最高累计30日，如中间间断打卡，则重新从第1日开始计算</view>
					<view>现金福利直接发放到余额（可直接参与抢购）</view>
				</view>
			...
			<view class='signTip acea-row row-center-wrapper' :class='active==true?"on":""'>
				...
				<view class='signTipCon' :style="'background-image: url('+domain+'/static/diy/signSuccess'+keyColor+'.png)'">
					...
					<view class='integral'>获得{{integral}}余额</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			scrollTo(num) {
				const box = uni.createSelectorQuery().selectAll('.scroll-item')
				let deviceWidth = uni.getSystemInfoSync().windowWidth;
				box.boundingClientRect().exec(data => {
					let el = data[0][num - 1];
					this.offsetLeft = this.scrollLeft + el.left - deviceWidth / 2 + el.width / 2;
				})
...
<style scoped lang="scss">
	...
	.sign .wrapper .list .item .venus.venusSelect+.num {
		color: #ff9000;
	}
