<template>
	<view :style="viewColor">
			...
			<view class='addAddress'>
				<view class='list'>
					...
					<view class='item acea-row row-between-wrapper'>
						...
						<input type='text' class="location-input" placeholder='请填写具体地址' name='detail' placeholder-class='placeholder'
		...
		<areaWindow ref="areaWindow" :display="display" :address="addressInfo" @submit="OnChangeAddress" @changeClose="changeClose">
		</areaWindow>
			...
			getWxAddress: function() {
				...
				uni.authorize({
					...
					success: function(res) {
						uni.chooseAddress({
							success: function(res) {
								getCityList(res.provinceName + '/' + res.cityName + '/' + res.countyName).then(
									res => {
				...
				uni.getLocation({
					...
					success: (res) => {
						...
						getGeocoder({
							...
							const data = res.data;
							getCityList(data.address_component.province + '/' + data.address_component.city + '/' + data
								.address_component.district + '/' + (!data.address_reference.town ? '' : data
									.address_reference.town.title)).then(res => {
				...
				editAddress(value).then(res => {
					...
					setTimeout(function() {
						if (that.cartId) {
							...
							uni.navigateTo({
								url: '/pages/users/order_confirm/index?cartId=' + cartId + '&addressId=' + (that
									.id ? that.id : res.data
