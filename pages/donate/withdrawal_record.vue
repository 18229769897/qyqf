			...
			<view class='sign-record'>
				<block v-for="(item,index) in recordList" :key="index" v-if="recordList.length>0">
					<view class='list'>
						<view class='item'>
							<view class='listn'>
								<view class='itemn acea-row row-between-wrapper' style="height:auto;box-sizing: content-box; padding-bottom: 10rpx;padding-top:20rpx;">
									<view>
										...
										<template v-if="item.status==-1">
											<view class='name line1 fail' style="width: auto;">
												<view>结算失败，金额已退回至{{type==2?"现金钱包":"收益钱包"}}</view>
