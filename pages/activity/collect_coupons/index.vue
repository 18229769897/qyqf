<template>
	<view class='my-coupon' :style="viewColor">
		...
		<view class="listBox">
			<view class="list acea-row row-around" v-for="(item) in list" :key='item.coupon_id'>
				<view class="left cross"  v-if="item.ProductLst.length == 1">
					<view class="cross-left" v-for="(items) in item.ProductLst" :key='items.spu_id'>
				...
				<view class="left" v-else>
					...
					<view class="info">
						<view class="info-box" v-for="(items) in item.ProductLst" :key='items.spu_id'>
							<easyLoadimage :image-src="items.image"></easyLoadimage>
				...
				<view class="right">
					...
					<view class="title">
						...
						<view class="btn" v-if="!item.issue" @click="receiveCoupon(item)">立即领取</view>
						<!-- <view class="btn" v-if="item.issue">去使用</view> -->
						<navigator :url="'/pages/columnGoods/goods_coupon_list/index?coupon_id='+item.coupon_id" class='btn shiyong' hover-class="none" v-if="item.issue">
						去使用
						</navigator>
					</view>
				</view>
