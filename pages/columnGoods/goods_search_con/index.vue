							...
							<view class="left-wrapper">
								...
								<view class="con-box">
									<view class="name acea-row row-between-wrapper line1">
										...
										<text v-if="item.type_name" class="font-bg-red bt-color">{{item.type_name}}</text>
										<text v-else-if="item.is_trader" class="font-bg-red bt-color">自营</text>
									</view>
									<view class="star-box">
										<view class="star">
...
<script>
	...
	export default {
		...
		computed: {
			...
			...configMap({hide_mer_status:0},mapGetters(['uid','isLogin','scrollTop','viewColor','keyColor'])),
		},
		// 滚动监听
		onPageScroll() {
			uni.$emit('scroll');
