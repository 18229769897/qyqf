<template>
	<view :style="viewColor">
		...
		<view class="new-users skeleton" :style="{visibility: showSkeleton ? 'hidden' : 'visible'}">
				...
				<view class="user-card">
					...
					<view class="user-info">
						<view class="realtive">
							...
							<image v-if="userInfo.is_green==1" src="@/static/images/green-vip1.png" style="width: 36rpx;position: absolute;right: 10rpx;bottom: 10rpx;"
								mode="widthFix">
							</image>
							<image v-if="userInfo.is_green==3" src="@/static/images/green-vip2.png" style="width: 36rpx;position: absolute;right: 10rpx;bottom: 10rpx;"
								mode="widthFix">
							</image>
						</view>
						...
						<view class="info">
							...
							<view class="name" v-if="userInfo.uid">
								<text class="mr-1">{{userInfo.nickname}}</text>
								<image src="@/static/images/hehuoren.png" v-if="userInfo.partner==1" style="width: 34rpx;" mode="widthFix"></image>
							</view>
							<view class="user-level-box">{{levelMap[userInfo.level]}}</view>
							<view class="phone" v-if="!userInfo.phone && isLogin" @tap="bindPhone">绑定手机号</view>
						</view>
					</view>
					<view class="right-btn">
						<view class="iconfont icon-shezhi" v-if="isLogin" @click="toPage(`/pages/users/user_info/index`)"></view>
			...
			<view class="wrapper">
				<view class="order-wrapper">
					<view class="order-hd flex skeleton-rect">
						...
						<view class="right flex" @click="authTo('/pages/users/order_list/index')" hover-class="none" open-type="navigate">
							全部订单
					...
					<view class="order-bd">
						<block v-for="(item,index) in orderMenu" :key="index">
							<view class="order-item" @click="authTo(item.url)" hover-class="none">
								<view class="pic">
									<image :src="domain+item.img+keyColor+'.png'" mode="" class="skeleton-radius"></image>
				...
				<view class="user-menus" style="margin-top: 20rpx;">
					...
					<view class="menu-box">
						...
						<view class="item" v-if="userInfo.service && userInfo.service.is_verify">
							<view @click="authTo(`/pages/admin/order_cancellation/index`)" class="item-count">
								<image src="@/static/images/hexiao.png"></image>
								<text>订单核销</text>
							</view>
						</view>
						<view class="item">
							<view @click="authTo(`/pages/store/settled/index`)" class="item-count">
								<image src="@/static/images/store.png"></image>
								<text>商家入驻</text>
							</view>
						</view>
						
						<view class="item" v-if="userInfo.is_president==1">
							<view @click="toPage(`/pages/zongcai/index`)" class="item-count">
								<image src="@/static/images/zongcai.png"></image>
								<text>总裁中心</text>
							</view>
						</view>
					</view>
				</view>
				<!-- <view class="menus-list-item" v-if="!userInfo.topService && userInfo.service" @click="toService(1)">
				...
				<view class="menus-list-item" v-if="!userInfo.service && userInfo.topService" @click="toService(0)">
					<view class="item-text">
						<view class="title">
			...
			<authorize @onLoadFun="onLoadFun" :isAuto="isAuto" :isShowAuth="isShowAuth" @authColse="authColse" :isGoIndex="false">
			</authorize>
			<!-- #ifndef H5 -->
	...
	export default {
		...
		methods: {
			...
			filterMenus: function(item) {
				...
				} else if (item.key == '/pages/store/settled/index') item.isShow = that.mer_intention_open == 1 && that.hide_mer_status !=
					1
			...
			showMenu(menu) {
				return !menu || ['integral', 'service', 'admin_order', 'verify_order', 'intention', 'promoter', 'balance'].indexOf(menu) === -
					1;
			},
			goUrl(url) {
				if (url.indexOf("http") != -1) {
					...
					if (['/pages/goods_cate/goods_cate', '/pages/order_addcart/order_addcart', '/pages/user/index', '/pages/plant_grass/index']
						.indexOf(url) == -1) {
						uni.navigateTo({
							url: url
						})
			...
			getUserInfo: function() {
				...
				getUserInfo().then(res => {
					...
					this.$store.commit("UPDATE_USERINFO", that.userInfo);
			...
			getMyMenus: function() {
				...
				getMenuList().then(res => {
					this.personalMenu = res.data.menu;
				});
			},
...
<style lang="scss">
	.new-users {
		.head {
			background-image: linear-gradient(to right, #5ab87f, #339064);
