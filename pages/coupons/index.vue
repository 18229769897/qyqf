		...
		<view class="main">
			<view v-show="tabIndex==0">
				<view class="form-item">
					...
					<payType :list="[`weixin`,`now_money`]" :payType.sync="payType"></payType>
...
<script>
	...
	import { rechargeWechat2, giveHaiHui } from "@/api/user.js";
	export default {
		data() {
			return {
				tabIndex: 0,
		...
		methods: {
			...
			async give() {
				...
				if (Number(f.count) > this.userInfo.coupons) return this.$msg.fail("焕际值余额不足");
				if (!(await this.$msg.alert("是否确定转赠焕际值"))) return;
				try {
					let { message } = await giveHaiHui(f);
					this.$store.dispatch("USERINFO");
