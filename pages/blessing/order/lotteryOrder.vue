			...
			<view class='header bg-color'>
				<view class='picTxt acea-row row-between-wrapper'>
					<view class='text'>
						...
						<view>消费订单：{{orderData.orderCount || 0}} 总消费：{{orderData.orderPrice || 0}}积分</view>
					</view>
					<view class='pictrue'>
							...
							<view class='item-info acea-row row-between row-top' v-for="(order,j) in item.orderList" :key="order.order_id+j">
								<block v-for="(goods,g) in order.orderProduct">
									...
									<view class='text acea-row row-between'>
										...
										<view class='money'>
											<view>{{goods.cart_info.productAttr.price}}积分</view>
						...
						<view class='item' :key="index">
							<view @click='goOrderDetails(item.order_id)'>
								<view class='title acea-row row-between-wrapper'>
									...
									<view v-if="item.status == 0 && item.user_check==1">待发货</view>
									<view v-if="item.status == 1">待收货</view>
									<view v-if="item.status == 2">待评价</view>
									<view v-if="item.status == 3" style="color:#27ae60;">已完成</view>
								</view>
								...
								<view class='totalPrice'>共{{item.orderNum || 0}}件商品，总金额：
									<text class='money font-color'>{{item.pay_price}}积分</text>
								</view>
							</view>
							<view class='bottom acea-row row-right row-middle'>
								...
								<block v-if="item.status == 2">
									<view class='bnt cancelBnt' @click='goOrderDetails(item.order_id)'>查看详情</view>
									<view class='bnt bg-color' @click='goOrderDetails_Evaluation(item.orderProduct[0].order_product_id,item.order_id)'>去评价</view>
								</block>
...
<script>
	...
	export default {
		...
		methods: {
			// 兑换积分
			async tradePoint(orderID) {
				uni.showModal({
					title: '提示',
					content: '您确定要将该笔订单兑换为积分吗？',
					success: async (res) => {
						if (res.confirm) {
							uni.showLoading({
								title: " "
			...
			goOrderDetails: function(order_id) {
				...
				openOrderSubscribe().then(() => {
					...
					if (self.orderStatus == 0) {
						...
						uni.navigateTo({
							url: `/pages/order_details/index?order_id=${order_id}&type=5`
						})
					}
				}).catch(() => {
					uni.hideLoading();
			...
			goOrderDetails_Evaluation: function(order_id) {
				...
				if (self.orderStatus == 0) {
					...
					uni.navigateTo({
						url: `/pages/order_details/index?order_id=${order_id}&type=2`
					})
				}
				// #endif  
				// #ifndef MP
				uni.navigateTo({
					url: `/pages/users/goods_comment_con/index?uni=${order_id}&order_id=${order_id}`
				})
				// #endif
			},
			/**
...
<style scoped lang="scss">
	...
	.my-order .nav .item.on {
		font-weight: bold;
		border-bottom: 5rpx solid #0f734f;
	}
