...
// 主题色
const $mainColor = "fe3661";
// 弹窗图标url
const $iconUrl = require("../static/ic_ar.png");
...
import { appUpdateApi } from "@/api/api.js"


export default () => {
	plus.runtime.getProperty(plus.runtime.appid, async (wgtinfo) => {
		let { data } = await appUpdateApi();
		let wgt = data.find(i => i.update_type == 2);
		let apk = data.find(i => i.update_type == 1);

		let webAppVer = apk.versions // 线上整包版本
		let webWgtVer = wgt.versions // 线上wgt版本
		let localAppVer = plus.runtime.version // 本地整包版本
		let localWgtVer = wgtinfo.version; // 本地wgt版本

		let isUpdate_app = versionCheck(webAppVer, localAppVer);
		let isupDate_wgt = versionCheck(webWgtVer, localWgtVer);

		let params;
		if (isUpdate_app && (osMap[apk.target] == "all" || osMap[apk.target] == platform)) {
			// 整包更新
			console.log("有更新：" + webAppVer);
			params = {
				versionName: webAppVer, // 版本名称 如1.0.0
				versionInfo: apk.app_intro, // 更新描述
				updateType: "forcibly", // forcibly:强制更新, solicit:弹窗确认更新, silent:静默更新
				downloadUrl: apk.path // 更新包下载地址
			}
			updatePopup(params, () => {
				if (platform == "ios") {
					let appleId = "1637811479";
			...
			Cache.set("isUpdateHuanJiApp", 1);
		} else if (isupDate_wgt && (osMap[wgt.target] == "all" || osMap[wgt.target] == platform)) {
			// 热更新
			console.log("有更新：" + webWgtVer);
			params = {
				versionName: webWgtVer,
				versionInfo: wgt.app_intro,
				updateType: "forcibly",
				downloadUrl: wgt.path
			}
			updatePopup(params, () => {
				getDownload(params);
			});
	...
	popupView.addEventListener("click", function(e) {
		...
		if (e.clientY > maxTop - 40 && e.clientY < maxTop) {
			if (buttonNum == 1) {
				...
				if (e.clientX > popupViewData.viewContentPadding && e.clientX < maxLeft - buttonWidth - popupViewData
					.viewContentPadding) {
					maskLayer.hide();
					popupView.hide();
